"""
 Bite 88. Write a performance bite_088ing context manager  
...
import bite_088 
from bite_088 import timeit, ALERT_MSG


@pytest.fixture()
def clean_cache():
    """Make sure each test starts with a clean cache dict"""
    bite_088.violations = defaultdict(int)


@patch('bite_088.time', MagicMock(side_effect=[0, 2]))
...
@patch('bite_088.time', MagicMock(side_effect=[0, 2, 0, 3]))
...
@patch('bite_088.time', MagicMock(side_effect=[0, 2, 0, 3, 0, 4]))
...
@patch('bite_088.time', MagicMock(side_effect=[0, 2, 0, 3, 0, 4, 0, 5]))
...
@patch('bite_088.time', MagicMock(side_effect=[0, 3, 0, 3, 0, 4, 0, 5]))
def test_four_operations_took_too_long_but_on_two_days(clean_cache, capfd):
    ...
    with patch('bite_088.get_today', return_value=date(2018, 5, 1)):
        with timeit():
            pass
        with timeit():
            pass
    # 2 violations today
    with patch('bite_088.get_today', return_value=date(2018, 5, 2)):
        with timeit():
            pass
        with timeit():
            pass
