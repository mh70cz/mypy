...
def high_low_record_breakers_for_2015():
    ...
    daily_max = grp_tmax.agg({"ID": "first",
            "Data_Value": np.max,
           })
    daily_min = grp_tmin.agg({"ID": "first",
        "Data_Value": np.min,
       })
    ...
    md_max_05_14 = grp_md_max_05_14.agg({"ID": "first", "Date": "first", "Data_Value": np.max})
    md_min_05_14 = grp_md_min_05_14.agg({"ID": "first", "Date": "first", "Data_Value": np.min})
...
"""
grp_md_max_05_15.agg({"ID": "first", "Date": "first", "Data_Value": np.max})
gm = g.agg({"ID": "first",
            "Data_Value": np.max,
           })
    
grp_md_04_15.agg({"ID": "first", "Data_Value": np.max, "md": "first"}).reset_index(inplace=True)
