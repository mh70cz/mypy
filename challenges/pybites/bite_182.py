...
def extract_quotes(html: str = HTML) -> dict:
    ...
    quotes = dict()    
    #patt_raw = re.compile("(?<=<p>).*(?=</p>)") #
    patt_raw = re.compile(r"<p>\d.*</p>")
    raw_quotes = patt_raw.findall(html)
    for rq in raw_quotes:
        patt_q = re.compile(r'(?<=").*(?=")')
        patt_a = re.compile(r"(?<=\s-\s).*(?=</p>)")
        quote = patt_q.search(rq).group()
        author = patt_a.search(rq).group()
        quotes[author] = quote
    return quotes