"""
@author: mh70; 2018-07-18; Bite 36. Having fun with *args and **kwargs 
https://codechalleng.es/bites/36/ 

Invoke pytest from current module 
pytest run from current file
https://stackoverflow.com/questions/35353771/invoke-pytest-from-python-for-current-module-only
"""

import pytest
#pytest.main(["bite_36.py"])  
...
def get_profile(name, age, *sports, **awards):
    ...
    out_d["age"] = age
    if sports:
        out_d["sports"] = sorted(list(sports))
    if awards:        
        out_d["awards"] = awards
