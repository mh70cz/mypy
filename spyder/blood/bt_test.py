...
def test_AB_POS_can_donate_to_own_group_only_numeric_input():
    ...
    for i in range(7):
        recipient = i
        assert check_bt(donor, recipient) == False
...
def test_red_blood_cell_incompatibility():
    assert check_bt(Bloodtype.B_POS, Bloodtype.B_NEG) == False
    assert check_bt(Bloodtype.A_NEG, Bloodtype.B_NEG) == False
    assert check_bt(Bloodtype.AB_NEG, Bloodtype.B_POS) == False
    assert check_bt(Bloodtype.B_NEG, Bloodtype.A_POS) == False
...
def test_red_blood_cell_incompatibility_text_input():
    assert check_bt("0+", "A-") == False
    assert check_bt("0+", "B-") == False
    assert check_bt("B-", "0-") == False
    assert check_bt("AB-", "A+") == False   
